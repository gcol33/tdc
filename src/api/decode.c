...
tdc_status tdc_decode_block(const uint8_t *src, size_t src_size,
    ...
    /* Resolve vtables. The entropy chain is resolved below; the model
     * is needed up front for its residual-dtype lookup. */
     ...
     * implicit NONE passthrough. */
    if (entropy_len == 0) {
        /* Passthrough: payload IS the residual stream. */
        ...
        /* Cross-check innermost stage input size against the header. */
        ...
        /* bufs[cur] now holds the residual stream of size
         * hdr.uncompressed_size — the reverse transform chain below
         * reads from it as before. */
