...
void tdc_dp_dump(const char *label) {
    ...
    fprintf(stderr,
        "[decode-profile %s] %llu calls, %llu seqs, %llu cycles total\n",
        label,
    ...
    for (int i = 0; i < TDC_DP_NSEC; i++) {
        ...
        double cps = tdc_dp_n_seqs_total
            ...
            : 0.0;
        fprintf(stderr, "  %-8s %14llu cyc  %5.1f%%  %7.2f cyc/seq\n",
                names[i],
