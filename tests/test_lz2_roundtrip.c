/*
 * tests/test_lz2_roundtrip.c
 *
 * Round-trip test for the TDC_ENTROPY_LZ2 backend.
 *
...
/* Resolve the lz2 vtable directly (not via tdc_entropy_get) so this test
...
static int roundtrip_check(const uint8_t *src, size_t n, const char *label) {
    ...
    tdc_status st = tdc_entropy_lz2_vt.encode(src, n, NULL, &enc);
    ASSERT_OR_DIE(st == TDC_OK, "encode failed");
    ASSERT_OR_DIE(enc.size <= tdc_entropy_lz2_vt.encode_bound(n),
                  "encode_bound exceeded");

    uint8_t *dec = (uint8_t *)malloc(n ? n : 1);
    ASSERT_OR_DIE(dec != NULL, "decode buffer alloc failed");

    st = tdc_entropy_lz2_vt.decode(enc.data, enc.size, dec, n);
...
int main(void) {
    ...
    {
        tdc_buffer enc = make_buffer();
        tdc_status st = tdc_entropy_lz2_vt.encode(src + N / 2, N / 2, NULL, &enc);
    ...
    /* 4. Edge cases: empty, 1 byte, 3 bytes (below LZ2_MIN_MATCH+1). */
