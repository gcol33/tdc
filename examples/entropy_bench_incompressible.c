/* docs/examples/entropy_bench_incompressible.c
 *
 * Four entropy backends on 1 MiB of LCG noise. The stream has no
 * repeats, no byte-distribution skew the symbol coders can exploit;
 * every backend either returns the input almost verbatim or grows it.
 * The numbers serve as the "floor" calibration for the benchmark table
 * in docs/backends/entropy.md.
 *
...
static void bench(const char *label, tdc_entropy_id id,
    ...
    double enc_med = median5(enc_times);
    double dec_med = median5(dec_times);
    double mb = (double)raw / 1.0e6;
    printf("  %-10s enc=%8zu  ratio=%6.2fx  enc=%6.0f MB/s  dec=%6.0f MB/s\n",
...
int main(void) {
    ...
    printf("incompressible input: %d bytes LCG noise\n", N);
