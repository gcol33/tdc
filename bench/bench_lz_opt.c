/*
 * bench/bench_lz_opt.c
 *
 * Side-by-side throughput + ratio comparison for TDC_ENTROPY_LZ (greedy)
 * vs TDC_ENTROPY_LZ_OPT (forward-DP optimal parser) on the entropy stage
 ...
 * SHUFFLE_LZ pipeline feeds the entropy stage on structured tabular data. */
...
static int run_case(const char *label, const uint8_t *src, size_t n) {
    ...
    int rc;
    rc = bench_encode(&tdc_entropy_lz_vt,     src, n, ITERS_ENC_GREEDY, &g, &g_enc_secs);
    if (rc) { fprintf(stderr, "FAIL [%s]: greedy encode -> %d\n", label, rc); free_buffer(&g); free_buffer(&o); return 1; }
    rc = bench_encode(&tdc_entropy_lz_opt_vt, src, n, ITERS_ENC_OPT,    &o, &o_enc_secs);
    ...
    rc = bench_decode(&tdc_entropy_lz_vt, g.data, g.size, dec, n, ITERS_DEC, &g_dec_secs);
    if (rc || memcmp(dec, src, n) != 0) {
        fprintf(stderr, "FAIL [%s]: greedy decode/round-trip\n", label);
        free(dec); free_buffer(&g); free_buffer(&o); return 1;
    }
    rc = bench_decode(&tdc_entropy_lz_vt, o.data, o.size, dec, n, ITERS_DEC, &o_dec_secs);
    if (rc || memcmp(dec, src, n) != 0) {
...
int main(void) {
    ...
    printf("\n");
    printf("bench_lz_opt: greedy vs forward-DP optimal LZ (entropy stage, isolated)\n");
    ...
    printf("\n");
    if (any_fail) {
        printf("bench_lz_opt: FAIL (invariant or round-trip)\n");
        return 1;
    }
    printf("bench_lz_opt: OK\n");
    return 0;
}
