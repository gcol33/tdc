/* docs/examples/entropy_bench_mixed.c
 *
 * Realistic mixed workload: 512x512 i16 DEM-style raster with four
 * planar quadrants plus small jitter, encoded through PRED_2D/PAETH +
 * ZIGZAG + BYTE_SHUFFLE, then handed to each entropy backend. The input
 * to the entropy stage is the post-shuffle residual byte stream, which
 * is what LZ / Huffman / FSE see in a real pipeline.
 *
...
static void bench(const char *label, tdc_entropy_id id,
    ...
    printf("  %-10s enc=%7zu  ratio=%8.2fx  enc=%6.0f MB/s  dec=%6.0f MB/s\n",
...
int main(void) {
    enum { H = 512, W = 512, N = H * W };
    static int16_t raster[N];

    /* Four planar quadrants with different plane coefficients + small
     * jitter. PAETH predicts exactly on the interior of each quadrant,
     * so the residual concentrates on the quadrant seams and the jitter. */
    uint32_t lcg = 1u;
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x < W; ++x) {
            lcg = lcg * 1103515245u + 12345u;
            int jitter = (int)((lcg >> 16) & 0x07) - 3;    /* [-3, 4] */
            int a, b, c;
            if (y < H/2 && x < W/2)      { a = 10;  b = 3;  c = 5;  }
            else if (y < H/2)            { a = 200; b = -2; c = 4;  }
            else if (x < W/2)            { a = 500; b = 1;  c = -3; }
            else                         { a = 80;  b = 2;  c = 2;  }
            raster[y * W + x] = (int16_t)(a + b * x + c * y + jitter);
        }
    }
    ...
    printf("mixed input: %zu bytes (%dx%d i16 DEM + jitter, PAETH+ZZ+BSHUF)\n",
           raw, H, W);
